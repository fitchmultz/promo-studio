...
Promo Studio helps commerce teams and evaluators see how Codex can turn a campaign brief into a tested, reviewable storefront variant instead of a throwaway text mockup.

[![Watch the Promo Studio walkthrough](https://img.youtube.com/vi/zPRLtiP8v7w/maxresdefault.jpg)](https://youtu.be/zPRLtiP8v7w)

## What you are seeing

A user picks a campaign goal for the Ribbed Market Tote, clicks **Create Variant**, and Promo Studio runs Codex as a commerce code agent:

- Codex receives a real software task, not a copywriting prompt.
- A fresh storefront workspace is copied from `templates/storefront` for that run.
- Codex is instructed to edit source files, run `npm test`, run `npm run build`, and write a manifest that reports the outcome.
...
- **Evaluators and reviewers** who want proof that the demo uses Codex to modify and validate code, not just generate UI text.
...
| --- | --- | --- |
| "Did Codex actually edit code?" | Creates `codex-workspaces/run-<id>/storefront` and runs Codex with `workspace-write` access to that isolated copy. | `lib/workspace.ts`, `lib/codex-runner.ts`, `/runs/<id>` transcript and diff tabs |
| "Can I see the agent work live?" | Normalizes streamed Codex events into JSONL and renders them while the run is active. | `components/ActivityStream.tsx`, persisted `VariantRun.transcript` |
...
| "Is this easy to inspect locally?" | Seeds a demo user, product, and completed example receipt so the review UI works before a live Codex run. | `prisma/seed.ts`, `npm run setup`, `/proof` |

## Fastest way to see it work

Prerequisites: Node.js `>=20.19.0` and npm. The seeded proof path does not require a `.env` file or live Codex credentials.
...
Expected local result: the app starts with a seeded Ribbed Market Tote product and a completed `seeded-demo-variant` receipt. Creating a new live variant also requires a working local Codex auth setup through subscription or API-key mode.

## Run a live Codex variant

After setup, go to `/studio`, choose a campaign goal, optionally edit the brief, and click **Create Variant**.

A live run is designed to:

1. Copy `templates/storefront` into `codex-workspaces/run-<id>/storefront`.
2. Run Codex through the TypeScript SDK by default.
3. Stream activity to the run page.
4. Instruct Codex to run `npm test` and `npm run build` in the storefront workspace.
...
- `CODEX_MODEL`, `CODEX_REASONING_EFFORT`, and `CODEX_TIMEOUT_MS` tune the run.
...
POST /api/variant-runs
  ...
  -> copy templates/storefront into codex-workspaces/run-<id>/storefront
...
- Codex SDK runtime with `codex exec` fallback.
...
- Live variant creation depends on local Codex credentials and network/runtime availability.
- Generated workspaces are local artifacts under `codex-workspaces/` and can be cleared with `npm run reset:workspaces`.
- The storefront is intentionally small so the Codex change, validation path, and review surface stay easy to inspect.
...
| `lib/codex-runner.ts` | Codex SDK/exec runtime lifecycle, JSONL streaming, manifest parsing, validation, and persistence |
...
| `templates/storefront/` | Vite storefront template that Codex modifies during each isolated run |
...
Run the quick start, sign in with the seeded demo account, and open `/proof` first. It shows the receipt shape that every live Codex variant must earn before you inspect or share the generated storefront.
