...
When `agentCore=cursor`, Promo Studio runs storefront variants through the **Cursor TypeScript SDK** (`@cursor/sdk` 1.0.22, pinned exactly) using a local agent scoped to the isolated workspace copy.
...
## @cursor/sdk 1.0.22 changes incorporated

Evidence reviewed: npm registry metadata for `@cursor/sdk@1.0.22`, installed type declarations, and the current Cursor TypeScript SDK docs at <https://cursor.com/docs/sdk/typescript>.

Observed upstream change inventory through 1.0.22:

- Package metadata and optional native `@cursor/sdk-<platform>` package pins now resolve to `1.0.22`; declared runtime dependencies remain compatible with the existing server-side integration.
...
- 1.0.22 keeps the public run lifecycle/store declaration surfaces (`run-event-notifier-api`, `run-store-public-types`, and vendored shared type declarations). Promo Studio does not need those APIs for one-shot storefront runs.
