...
When `agentCore=pi`, the integration uses `@earendil-works/pi-coding-agent` v0.76.0 or newer for required automation, recommends v0.78.1 or newer for current Pi best practices, and runs `pi --mode json` as a subprocess. Promo Studio passes the prompt on stdin, sets `--session-id <run-id>`, stores sessions under gitignored `artifacts/pi-sessions`, and forwards only safe runtime plus Pi/provider environment variables to the child process.
