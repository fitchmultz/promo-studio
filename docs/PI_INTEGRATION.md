...
Pi v0.80.1 is the required floor for local development and Pi automation runs. The doctor fails when the installed CLI or SDK package is below v0.80.1 so local runs match the current package floor.

SDK and RPC were reviewed against the v0.80.1 docs. The SDK is preferred for same-process Node integrations and RPC is preferred for long-lived custom clients, but Promo Studio intentionally keeps Pi in a subprocess for run isolation, CLI extension parity, and a one-shot prompt lifecycle.
