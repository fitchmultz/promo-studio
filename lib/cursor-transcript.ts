...
function isSdkMessage(value: Record<string, unknown>): value is SDKMessage & Record<string, unknown> {
	return (
...
export function cursorStreamEventToTranscriptPayload(
	event: unknown,
): unknown {
	if (isJsonObject(event) && isSdkMessage(event)) {
		return event;
	}
	const sdkMessage = localRunStreamEventToSdkMessage(event as LocalRunStreamEvent);
